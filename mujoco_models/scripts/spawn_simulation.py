...
class SpawnSimulation(object):
    ...
    def __init__(self, new_xml_filename):
        self._new_xml_filename = new_xml_filename
        self._package_dir = rospkg.RosPack().get_path("mujoco_models")
        self._xml_config_dir = self._package_dir + "/urdf"
    ...
    def _spawn_sim_environment_service(self, req):
        ...
        for obj in req.objects.objects:
            ...
            try:
                ...
                rospy.logerr("Could not append object")
                spawned = False
        try:
            subprocess.call("roslaunch mujoco_ros_control mujoco_simulation.launch sim:=true grasp_controller:=true \
                             robot_model_path:={}/{}".format(self._xml_config_dir, self._new_xml_filename), shell=True)
        except:
    ...
    def _append_object_to_xml(self, obj_name, obj_pose):
        ...
        for child in self._base_configuration_xml.getroot():
            if child.tag == "asset":
                ...
                mesh_tag = xmlTool.SubElement(child, "mesh", {'name': obj_name, 'file': obj_name[:-2] + '.stl'})
            if child.tag == "worldbody":
                ...
                geom_tag = xmlTool.SubElement(body_tag, "geom", {'type': 'mesh', 'rgba': '0.7 0.7 0.7 1',
                                                                 'mesh': obj_name})
        self._base_configuration_xml.write("{}/{}".format(self._xml_config_dir, self._new_xml_filename))


if __name__ == '__main__':
    rospy.init_node("spawn_simulation_node")
    spawn_sim = SpawnSimulation("world_mujoco_test.xml")
    rospy.spin()
